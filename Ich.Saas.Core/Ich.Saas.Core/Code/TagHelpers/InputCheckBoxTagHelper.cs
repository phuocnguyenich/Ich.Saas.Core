...
namespace Ich.Saas.Core.Code.TagHelpers
{
    ...
    public class InputCheckBoxTagHelper : TagHelper
    {
        ...
        public InputCheckBoxTagHelper(IHtmlGenerator generator, IStringLocalizer<SharedResources> localizer)
        {
            Generator = generator;
        ...
        public override void Process(TagHelperContext context, TagHelperOutput output)
        {
            ...
            var label = _localizer[Label ?? Value?.Metadata?.DisplayName ?? Value?.Name ?? ""];
            var labelContent = $"<label for='{Value.Name}' class='col-sm-3 col-form-label'>{label}</label>";
